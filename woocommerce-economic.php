...
if ( ! function_exists( 'wce_logthis' ) ) {
    function wce_logthis($msg) {
        if(TESTING){
...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
    ...
    if ( ! class_exists( 'WCEconomic' ) ) {
        ...
        function wce_economic_enqueue(){
            wp_enqueue_script('jquery');
            wp_register_script( 'economic-script', plugins_url( '/js/economic.js', __FILE__ ) );
            wp_enqueue_script( 'economic-script' );
        }

        add_action( 'admin_enqueue_scripts', 'wce_economic_enqueue' );
        add_action( 'wp_ajax_sync_contacts', 'wce_economic_sync_contacts_callback' );
		
        function wce_economic_sync_contacts_callback() {
        ...
        add_action( 'wp_ajax_send_support_mail', 'wce_economic_send_support_mail_callback' );
		

        function wce_economic_send_support_mail_callback() {
		...
		function wce_economic_test_connection_callback() {
			include_once("class-economic-api.php");
			$wce = new WC_Economic();
			$wce_api = new WCE_API();
		...
		function wce_proversion() {
			?>
                <div class="error">
                    <p>Want to sync orders, invoices, products and inventory to e-conimic? Get the <button type="button button-primary" class="button button-primary" title="" style="margin:5px" onclick="window.open('http://whmcs.onlineforce.net/cart.php?a=add&pid=56&carttpl=flex-web20cart&language=English','_blank');">PRO version!</button></p>
                </div>
			<?php
		}
		
		add_action( 'admin_notices', 'wce_proversion' );
		//Proversion alert

        add_action( 'wp_ajax_test_connection', 'wce_economic_test_connection_callback' );


		//Section for wordpress pointers
		
		function wce_economic_wp_pointer_hide_callback(){
			update_option('economic-tour', false);
		}
		add_action( 'wp_ajax_wp_pointer_hide', 'wce_economic_wp_pointer_hide_callback' );
		
		$economic_tour = get_option('economic-tour');
		
		if(isset($economic_tour) && $economic_tour){
			// Register the pointer styles and scripts
			add_action( 'admin_enqueue_scripts', 'wce_enqueue_scripts' );
			
			// Add pointer javascript
			add_action( 'admin_print_footer_scripts', 'wce_add_pointer_scripts' );
		}
		...
		function wce_add_pointer_scripts()
		{
		...
		***********************************************************************************************************/
		...
		function wce_woo_is_economic_customer(WP_User $user) {
		  $is_customer = false;
		  foreach ($user->roles as $role) {
			wce_logthis("user role: " . $role);
			if ($role == 'customer') {
			  $is_customer = true;
			  break;
			}
		 ...
		 */
		add_action('update_user_meta', 'wce_update_user_meta_to_economic', 10, 4);
		function wce_update_user_meta_to_economic($meta_id, $object_id, $meta_key, $_meta_value) {
		  global $wpdb;
		  include_once("class-economic-api.php");
		  $wce = new WC_Economic();
		  $wce_api = new WCE_API();
		  if(in_array($meta_key, $wce_api->user_fields)){
			  wce_logthis("wce_update_user_meta_to_economic: meta_id: ".$meta_id." object_id: ".$object_id." meta_key: ".$meta_key." meta_value: ".$_meta_value);
			  $user = new WP_User($object_id);
			  if (wce_woo_is_economic_customer($user)) {
				...
				if($wce_api->woo_save_customer_meta_data_to_economic($user, $client, $meta_key, $_meta_value, $debtorHandle, $debtor_delivery_location_handle)){
					...
					wce_logthis("wce_update_user_meta_to_economic user: " . $user->ID . " additional data is synced with economic");
				}
				else{
					$wpdb->update ("wce_customers", array('synced' => 0), array('user_id' => $user->ID), array('%d'), array('%d'));
					wce_logthis("wce_update_user_meta_to_economic user: " . $user->ID . " additional data sync failed, please try again after sometime!");
				}
			  }
		  }else{
			  wce_logthis("wce_update_user_meta_to_economic: Not selected for sync, skipping meta_id: ".$meta_id." object_id: ".$object_id." meta_key: ".$meta_key." meta_value: ".$_meta_value);
		  }
		}
        ...
        class WC_Economic {
            ...
            public function __construct() {
                ...
                add_action( 'init', array( &$this, 'wce_load_settings' ) );
                ...
                add_action( 'admin_menu', array( &$this, 'wce_add_admin_menus' ) );


                // install necessary tables
                register_activation_hook( __FILE__, array(&$this, 'wce_install'));
                register_deactivation_hook( __FILE__, array(&$this, 'wce_uninstall'));
            }
             ...
             * @return bool
             */
            public function wce_install(){
                add_option('economic-tour', true);
				global $wpdb;
				$wce_customers = "wce_customers";
				
                $wpdb->query ("CREATE TABLE IF NOT EXISTS ".$wce_customers."( id mediumint(9) NOT NULL AUTO_INCREMENT,
